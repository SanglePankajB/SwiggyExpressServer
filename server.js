...
// app.get('/categories.php', (req, res) => {
//   res.json(
//     {
//         "categories": [
//             {
...
app.get('/categories.php', (req, res) => {
    ...
    fs.readFile(path.join(__dirname, 'Category.json'), 'utf8', (err, data) => {
        ...
        // Send the content of the JSON file as a response
...
app.get('/restaurents', (req, res) => {
    // Read the JSON file asynchronously
    fs.readFile(path.join(__dirname, 'Restaurants.json'), 'utf8', (err, data) => {
        if (err) {
            console.error('Error reading file:', err);
            return res.status(500).json({ message: 'Error reading the data file' });
        }
...
// This '0.0.0.0' will allow server to listen on all network interfaces not only localhost but all which are connected to the same network
